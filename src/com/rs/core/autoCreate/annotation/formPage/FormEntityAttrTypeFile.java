...
@Documented
public @interface FormEntityAttrTypeFile {
	// ------------------------------------基础属性--------------------------------------
	...
	// 第一个是showTitle,第二个是回显组件,第三个是禁用属性,第四个是objName,第五个是objName,第六个是http-request,第七个是before-upload,第八个是objName
	 ...
	 * 上传触发事件
	 */
	String onUploadHandle() default "%s_uploadHandle_file(param){  this.%s.file = param.file;this.form.%s = param.file;  }";

	/**
	 * 上传前事件 主要实现回显功能
	 */
	String onBeforeFunction() default "%s_uploadBeforeUpload_file(file){ var self = this,reader = new FileReader();reader.readAsDataURL(file);reader.onload = function(e) {self.%s.fileUrl = e.target.result;};  }";
	 ...
	 * 数据封装对象的名称
	 */
	String objName() default "el_upload_file";
